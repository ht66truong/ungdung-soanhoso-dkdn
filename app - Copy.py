...
def save_customer_profile(company_name, data):
    """Lưu profile khách hàng vào file JSON"""
    try:
        ensure_profiles_dir()
...
if 'ds_thanhvien' not in st.session_state:
    if cache_data and 'ds_thanhvien' in cache_data:
        ...
        st.session_state.ds_thanhvien = [{
            ...
            'quoctich': 'Việt Nam', 'dantoc': 'Kinh', 'diachi': '', 
            'vongop': '', 'thoihan': '90 ngày kể từ ngày cấp GCNĐKDN', 'taisan': 'Đồng Việt Nam', 'tyle': ''
        }]
...
if 'ds_huongloi' not in st.session_state:
    if cache_data and 'ds_huongloi' in cache_data:
        ...
        st.session_state.ds_huongloi = [{
            ...
            'quyen_chiphoi': '- Bổ nhiệm, miễn nhiệm GĐ/TGĐ\n- Sửa đổi điều lệ\n- Thay đổi cơ cấu tổ chức'
        }]
...
ORDER_LIST = [
    ...
    "dn_truso_sonha", "dn_truso_phuong", "dn_truso_quan", "dn_truso_tinh",
...
MAP_LABELS = {
    ...
    "dn_truso_sonha": "Số nhà, tên đường",
    "dn_truso_phuong": "Phường/Xã",
    "dn_truso_quan": "Quận/Huyện",
    "dn_truso_tinh": "Tỉnh/Thành phố",
    ...
    "ddpl_chucdanh": "Chức danh Người đại diện (GĐ, TGĐ...)"
}
...
if st.session_state.get('reset_all', False):
    ...
    if 'customer_select' in st.session_state:
        del st.session_state.customer_select
...
selected_files = st.sidebar.multiselect("Chọn file mẫu:", templates, default=[], key="selected_files")
...
if customer_list:
    ...
    if selected_customer:
        if st.sidebar.button("📂 Tải profile", key="btn_load_profile"):
            ...
            if profile:
                ...
                st.session_state.reset_customer = True  # Reset customer selection
                st.sidebar.success(f"✅ Đã tải: {selected_customer}")
                st.rerun()
else:
...
if selected_files:
    ...
    with tab2:
        ...
        d1, d2 = st.columns(2)
        for i, v in enumerate(ddpl_vars):
            with (d1 if i % 2 == 0 else d2):
                if 'gioitinh' in v:
                    context[v] = st.selectbox(f"ĐDPL - GIỚI TÍNH", ["Nam", "Nữ"], index=0 if st.session_state.context_cache.get(v, "Nam") == "Nam" else 1, key=f"in_{v}_{st.session_state.get('refresh_key', 0)}")
                else:
        ...
        if csh_vars:
            ...
            is_same = st.checkbox("🔄 CSH là Người đại diện (Đồng bộ dữ liệu)", value=False)
            if not is_same:
                c1, c2 = st.columns(2)
                for i, v in enumerate(csh_vars):
                    with (c1 if i % 2 == 0 else c2):
                        if 'gioitinh' in v:
                            context[v] = st.selectbox(f"CSH - GIỚI TÍNH", ["Nam", "Nữ"], index=0 if st.session_state.context_cache.get(v, "Nam") == "Nam" else 1, key=f"in_{v}_{st.session_state.get('refresh_key', 0)}")
                        else:
                            label = f"CSH - {v.replace('csh_', '').replace('_', ' ').upper()}"
                            context[v] = st.text_input(label, value=st.session_state.context_cache.get(v, ''), key=f"in_{v}_{st.session_state.get('refresh_key', 0)}")
    ...
    with tab3:
        ...
        for i, m in enumerate(st.session_state.ds_thanhvien):
            with st.expander(f"Thành viên {i+1}: {m['hoten']}", expanded=True):
                ...
                m['gioitinh'] = r3.selectbox("Giới tính", ["Nam", "Nữ"], index=0 if m['gioitinh']=="Nam" else 1, key=f"tv_g_{i}_{st.session_state.get('refresh_key', 0)}")
                
                r4, r5, r6 = st.columns(3)
                m['cccd'] = r4.text_input("CCCD/Định danh", value=m['cccd'], key=f"tv_c_{i}_{st.session_state.get('refresh_key', 0)}")
                ...
                m['vongop'] = r7.text_input("Vốn góp (VNĐ)", value=str(m['vongop']), key=f"tv_v_{i}_{st.session_state.get('refresh_key', 0)}")
        ...
        if st.button("➕ Thêm Thành viên", key=f"tv_add_{st.session_state.get('refresh_key', 0)}"):
            st.session_state.ds_thanhvien.append({'hoten': '', 'ngaysinh': '', 'gioitinh': 'Nam', 'cccd': '', 'quoctich': 'Việt Nam', 'dantoc': 'Kinh', 'diachi': '', 'vongop': '', 'thoihan': '90 ngày...', 'taisan': 'Đồng Việt Nam', 'tyle': ''})
            st.rerun()
    ...
    with tab4:
        ...
        for i, h in enumerate(st.session_state.ds_huongloi):
            with st.expander(f"CSHHL {i+1}: {h['hoten']}", expanded=True):
                ...
                h['gioitinh'] = h3.selectbox("Giới tính ", ["Nam", "Nữ"], index=0 if h['gioitinh']=="Nam" else 1, key=f"hl_g_{i}_{st.session_state.get('refresh_key', 0)}")
        ...
        if st.button("➕ Thêm CSHHL", key=f"hl_add_{st.session_state.get('refresh_key', 0)}"):
            st.session_state.ds_huongloi.append({'hoten': '', 'ngaysinh': '', 'gioitinh': 'Nam', 'cccd': '', 'quoctich': 'Việt Nam', 'dantoc': 'Kinh', 'diachi': '', 'tyle_vdl': '', 'tyle_bieuquyet': '', 'quyen_chiphoi': ''})
            st.rerun()
    ...
    st.divider()
    with st.expander("💼 Lưu/Quản lý Profile Khách Hàng", expanded=False):
        col_company, col_save_profile = st.columns([2, 1])
        with col_company:
            company_name = st.text_input("Tên khách hàng (để lưu profile):", 
                                        value=context.get('dn_tencongty', ''), 
                                        help="Nhập tên công ty để lưu profile khách hàng",
                                        key="input_company_name")
        with col_save_profile:
            if st.button("💾 Lưu Profile", use_container_width=True):
                if company_name:
                    profile_data = {
    ...
    # --- 5. XUẤT FILE & LƯU CACHE ---
    col_save, col_export, col_clear = st.columns([1, 2, 1])
    
    with col_save:
        if st.button("💾 LƯU TẠM", use_container_width=True, help="Lưu dữ liệu hiện tại mà không xuất hồ sơ"):
            cache_to_save = {
                'context_cache': context,
                'ds_thanhvien': st.session_state.ds_thanhvien,
                'ds_huongloi': st.session_state.ds_huongloi
    ...
    with col_export:
        if st.button("🚀 XUẤT HỒ SƠ", use_container_width=True):
            ...
            try:
                valid_vons = [float(m['vongop']) for m in st.session_state.ds_thanhvien if m['vongop'] and str(m['vongop']).strip() != ""]
                total_v = sum(valid_vons)
                for m in st.session_state.ds_thanhvien:
                    if m['vongop'] and total_v > 0:
                        m['tyle'] = round((float(m['vongop']) / total_v * 100), 3)
                    else:
                        m['tyle'] = ""
            except:
                pass
            
            context['ds_thanhvien'] = st.session_state.ds_thanhvien
    ...
    with col_clear:
        if st.button("🗑️ LÀM SẠCH", use_container_width=True, help="Xóa tất cả dữ liệu đã lưu"):
            clear_cache()
            st.session_state.context_cache = {}
            st.session_state.ds_thanhvien = [{
                'hoten': '', 'ngaysinh': '', 'gioitinh': 'Nam', 'cccd': '', 
            ...
            st.success("✅ Đã xóa toàn bộ dữ liệu lưu trữ")
            st.rerun()
else:
